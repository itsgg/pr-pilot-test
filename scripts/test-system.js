...
const SAMPLE_PR_INFO = {
  ...
  description:
    "This PR adds basic user management functionality including user creation, listing, and deletion.",
  author: "testuser",
  baseBranch: "main",
...
function testCostEstimation(fileDiffs, config) {
  ...
  try {
    ...
    console.log(
      `   Within cost cap: ${costEstimate.costUsd <= config.cost_cap_usd ? "Yes" : "No"}`,
    );
...
function testPromptGeneration(fileDiffs, config) {
  ...
  try {
    ...
    console.log(
      `   Total prompt length: ${(systemPrompt.length + userPrompt.length).toLocaleString()} characters`,
    );
...
async function testCommentFormatting() {
  ...
  try {
    const { CommentFormatter } = await import(
...
async function runSystemTests() {
  ...
  console.log(
    "This test verifies all core functionality without requiring real API credentials.\n",
  );
  ...
  try {
    ...
    if (passedTests === totalTests) {
      ...
      console.log("\n📝 Next steps:");
      console.log(
        "   1. Set up GitHub repository secrets (ANTHROPIC_API_KEY, GITHUB_TOKEN)",
      );
      console.log("   2. Copy .github/workflows/*.yml to your repository");
      console.log("   3. Create a test PR to verify the complete workflow");
      console.log(
        "   4. Monitor the metrics/run.json file for performance data",
      );
    } else {
