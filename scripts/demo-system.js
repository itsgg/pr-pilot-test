...
import {
  createSystemPrompt,
  createUserPrompt,
} from "../agent/prompts/review-prompt.js";
...
const MOCK_CLAUDE_RESPONSE = {
  summary:
    "This PR adds user authentication functionality but contains several security vulnerabilities, performance issues, and missing error handling that need to be addressed before merging.",
  issues: [
    {
      path: "src/auth.js",
      line: 3,
      ...
      severity: "high",
      explanation:
        "Hardcoded API key detected. This is a critical security vulnerability that could lead to unauthorized access.",
      fix_patch:
    ...
    },
    {
      ...
      severity: "high",
      explanation:
        "Sensitive data (password) included in JWT token payload. This exposes user credentials in the token.",
      fix_patch:
        "const payload = {\n  id: user.id,\n  email: user.email,\n  role: user.role\n};",
      confidence: 0.9,
    },
    {
      ...
      severity: "med",
      explanation:
        "JWT token has no expiration time, creating a security risk if tokens are compromised.",
      fix_patch:
    ...
    },
    {
      ...
      severity: "high",
      explanation:
        "Inefficient database simulation with O(n) loop creating 10,000 user objects. This will cause performance issues.",
      fix_patch:
        "// Use proper database query with pagination\nrouter.get('/users', async (req, res) => {\n  const { page = 1, limit = 50 } = req.query;\n  const users = await User.find()\n    .skip((page - 1) * limit)\n    .limit(limit);\n  res.json(users);\n});",
      confidence: 0.9,
    },
    {
      path: "src/routes/users.js",
      ...
      severity: "med",
      explanation:
        "Missing input validation for user creation endpoint. This could lead to invalid data being stored.",
      fix_patch:
    ...
    },
    {
      ...
      severity: "low",
      explanation:
        "Missing JSDoc comment for exported function. This reduces code maintainability.",
      fix_patch:
    ...
    },
    {
      ...
      severity: "med",
      explanation:
        "Test cases are empty and don't actually test functionality. This reduces code reliability.",
      fix_patch:
...
async function runSystemDemo() {
  ...
  console.log(
    "This demo shows the complete PR-Pilot system working with a realistic scenario.\n",
  );
  ...
  try {
    ...
    console.log(
      `✅ Configuration loaded: ${config.model} with $${config.cost_cap_usd} cost cap\n`,
    );
    ...
    fileDiffs.forEach((file, index) => {
      console.log(
        `   ${index + 1}. ${file.path} (${file.additions} additions, ${file.deletions} deletions)`,
      );
    });
    console.log();
    ...
    console.log(
      `   Within budget: ${costEstimate.costUsd <= config.cost_cap_usd ? "✅ Yes" : "❌ No"}\n`,
    );
    ...
    MOCK_CLAUDE_RESPONSE.issues.slice(0, 3).forEach((issue, index) => {
      ...
      console.log(
        `   ${index + 1}. ${issue.category.toUpperCase()} issue in ${issue.path}:${issue.line}`,
      );
    ...
    console.log("==============================");
    const issuesByCategory = MOCK_CLAUDE_RESPONSE.issues.reduce(
      (acc, issue) => {
