...
describe("PR Reviewer", () => {
  ...
  describe("constructor", () => {
    it("should create reviewer with default options", () => {
      ...
      assert.strictEqual(
        defaultReviewer.options.configPath,
        "config/agent.yaml",
      );
    ...
    it("should create reviewer with custom options", () => {
      ...
      assert.strictEqual(
        customReviewer.options.configPath,
        "custom-config.yaml",
      );
 ...
 app.listen(3000);`;
  ...
  describe("estimateReviewCost", () => {
    it("should estimate review cost correctly", async () => {
      ...
      try {
        const costEstimate = await reviewer.estimateReviewCost(
          mockFileDiffs,
          mockPrInfo,
        );
  ...
  describe("getAIReview", () => {
    it("should get AI review successfully", async () => {
      ...
      const reviewResponse = await reviewer.getAIReview(
        mockFileDiffs,
        mockPrInfo,
      );
      ...
      assert.strictEqual(
        reviewResponse.issues.length,
        mockReviewResponse.issues.length,
      );
      assert.strictEqual(
        reviewResponse.risks.length,
        mockReviewResponse.risks.length,
      );
    });
  });
  ...
  describe("postComments", () => {
    it("should post comments in dry run mode", async () => {
      ...
      const result = await reviewer.postComments(
        mockReviewResponse,
        123,
        "owner/repo",
      );
